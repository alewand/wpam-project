...
export const getWeek = (date: string = DateTime.local().toISO()): string[] => {
  ...
  const dates = Array.from({ length: 7 }, (_, i) => startOfWeek.plus({ days: i }).toISO() ?? "");
  return dates;
};
...
export const isDayInWeek = (day: string, week: string[]): boolean => {
  return week.some((dateIso) => {
    const date = DateTime.fromISO(dateIso);
    const targetDay = DateTime.fromISO(day);
    return date.hasSame(targetDay, "day");
  });
};
