...
export class MealsApiService {
  ...
  async getMealByBarcode(barcode: string): Promise<Meal> {
    ...
    return shouldBeReplaced && mealFromDB
      ? await this.mealsService.replaceMeal(mealFromDB.mealId, mappedMeal)
      : await this.mealsService.createMeal(mappedMeal);
  }
